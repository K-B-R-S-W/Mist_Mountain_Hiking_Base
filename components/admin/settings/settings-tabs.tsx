...
export function SettingsTabs({ settings, branding }: SettingsTabsProps) {
  ...
  const handleSubmit = async (formData: FormData) => {
    try {
      ...
      if (!result.ok) {
        ...
        toast.success("Global site settings saved successfully!");
      }
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Shared Form for Text & Configuration Settings */}
      {activeTab !== "branding" && (
        <form action={handleSubmit} className="card p-6 space-y-6 shadow-xs">
          ...
          <div className="pt-4 border-t border-black/8 flex justify-end">
            <ActionButton pendingLabel="Saving changes...">
