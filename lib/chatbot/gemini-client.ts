  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    try {
      ...
      if (response.status === 429 || response.status === 403 || response.status === 402) {
        console.warn(`Gemini API key #${i + 1} (${model}) hit rate-limit or quota error (${response.status}). Failing over to next key...`);
        continue;
      }

      if (!response.ok) {
        const errText = await response.text();
        console.warn(`Gemini API key #${i + 1} (${model}) returned status ${response.status}: ${errText}. Trying next key...`);
        continue;
      }
